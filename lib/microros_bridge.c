...
#define RX_PACKET_BYTES              480     // размер «полезной» части пакета от паблишера
#endif

#ifndef PREFETCH_PACKETS
#define PREFETCH_PACKETS             5       // сколько пакетов просим сразу
#endif

#ifndef MIN_FREE_PKTS_FOR_REQUEST
#define MIN_FREE_PKTS_FOR_REQUEST    3       // минимум свободных «слотов» в RB, чтобы слать need_packets
#endif

#ifndef RX_MSG_CAP_BYTES
#define RX_MSG_CAP_BYTES             2048    // вместимость msg.data (с запасом)
#endif
// ========================================================
...
static void sub_stream_cb(const void * msgin)
{
    ...
    // строго ожидаем RX_PACKET_BYTES
...
static void sub_draw_finish_cb(const void * msgin)
{
    ...
    // новые need_packets больше не слать, просто даём потребителю опустошить RB
...
static void log_task(void *arg)
{
    ...
    for(;;){
        ESP_LOGI(TAG,
                 ...
                 " seqmiss=%" PRIu32 " rb_used=%zu/%u outstanding=%u active=%d finish=%d",
                 s_stats.pkts_in,
                 s_stats.bytes_in,
                 s_stats.drops_total,
                 s_stats.seq_missed,
...
static void microros_init_task(void *arg)
{
    ...
    // Publisher: need_packets (BestEffort достаточно)
    ...
    // Subscribers: draw_start / draw_finish (control — default QoS)
    ...
    // Инициализируем приёмные сообщения
    ...
    xTaskCreatePinnedToCore(need_request_task,"mr_need_req",  4096, NULL, 5, NULL, tskNO_AFFINITY);
